...
- (instancetype)copyedObject {
  return [[JSONTree alloc] initWithObject:[self jsonObject]];
}

- (NSArray *)flit:(NSString *)keyword {
  NSMutableArray *result = [NSMutableArray new];
  for (JSONTree *node in self.children) {
    if ([node.description containsString:keyword]) {
      [result addObject:node];
    }
    [result addObjectsFromArray:[node flit:keyword]];
  }
  return result;
}

- (void)deleteEmptyNode {
  NSMutableArray *child = self.children.mutableCopy;
  for (JSONTree *node in child) {
    if (node.children.count == 0 && node.rawType != JSONTreeTypeKV) {
      [self deleteNode:node parent:nil];
    }else {
      [node deleteEmptyNode];
    }
  }
}
@end
